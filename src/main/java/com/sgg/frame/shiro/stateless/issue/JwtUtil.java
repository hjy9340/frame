...
public class JwtUtil {
    ...
    private static String issueJwt(String id, String subject, String issuer, Long period, String roles
        ...
        if(StrUtil.isNotBlank(roles)) jwt.claim("roles", roles);//角色
        if(StrUtil.isNotBlank(permissions)) jwt.claim("perms", permissions);//权限
        jwt.compressWith(CompressionCodecs.DEFLATE);//压缩，可选GZIP
        jwt.signWith(algorithm, secretKeyBytes);//加密设置
        return jwt.compact();
    }
    public static String hamcDigest(String data){
